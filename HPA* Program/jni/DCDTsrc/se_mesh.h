 ...
 *  Marcelo Kallmann 1996 - 2006
 *  
 *  The structure is based on circular lists of SeElement for vertex/edge/face nodes, 
 *  plus the connectivity information stored in the SeBase class.
 *
 *  Main Reference:
 *  M. Kallmann, "Navigation Queries from Triangulated Meshes", Motion in Games, 2010.
 *  This paper briefly describes the data structure and then makes an overview of the
 *  many triangulation methods implemented using the symedge data structure.
 *
 *  History:
 *
 *  08/2010 - This header file was edited to contain the MIG reference
 *  01/2006 - Now part of my new graphsim lib
 *  10/2004 - Becomes again SE lib, with dependency on a local (small) version of SR.
 *  10/2003 - Integration to the SR library, which is the newer version of FG.
 *  08/2002 - Triangulator now covers both constrained and conforming triangulations
 *  06/2001 - The library becomes again standalone
 *  09/2000 - Changed to always keep v/e/f information lists, added new se names 
 *  05/2000 - Finally operator mg done for converting triangle soups with genus>0
 ...
 *  07/1997 - Added Import() and Triangulation with virtual callbacks
...
# include "se_elements.h"

// ====================================== SeMeshBase ======================================

// SeMeshBase documentation in the end of this file
class SeMeshBase : public GsShared
 { public :
   ...
   private : // Data ===================================================================================
    ...
    gscbool _op_check_parms;        // Flag to tell if must check operators parameters (default false)
    gscbool _op_last_msg;           // Keeps the last result of an operator (OpMsg type)
    gscbool _output_op_errors;      // Flag that outputs all errors ocurred during operators (default true)
    int _vertices, _edges, _faces; // Elem counters
    semeshindex _curmark;          // Current values for marking
    gscbool _marking, _indexing;    // flags to indicate that marking or indexing is on
    GsManagerBase *_vtxman, *_edgman, *_facman; // keep used managers
  ...
  private : // Private methods ==========================================================================
   ...
   public : // General mesh functions ===================================================================
    ...
    /*! Inits an empty mesh. It requires to receive three manager classes
        ...
        the user can simply pass as argument "new GsManagerMeshElementManager", otherwise
    ...
    SeMeshBase ( GsManagerBase* vtxman, GsManagerBase* edgman, GsManagerBase* facman );
    ...
    bool is_triangle ( SeBase* e ) const { return e->nxt()->nxt()->nxt()==e; }

    /*! Returns the symedge adjacent to v1 and to the edge {v1,v2}, or null if the two 
        vertices do not form an edge, ie, are not connected */
    static SeBase* symedge ( SeVertex* v1, SeVertex* v2 );
   ...
   public :
    ...
    /*! Allows the user to safely mark elements with mark(), marked() and unmark()
        ...
        Attention: Marking and indexing can not be used at the same time.
        Also, end_marking() must be called when marking operations are finished. */
    void begin_marking ();
    ...
    /*! Returns true if marking is currently being used, otherwise false is returned. */
    gscbool marking () const { return _marking; }
    ...
    /*! Returns true if indexing is currently being used, otherwise false is returned. */
    gscbool indexing () const { return _indexing; }
   ...
   public : // Operators Control ===========================================================================
    ...
    /*! Get safe mode status. When it is true, error msgs are generated, but
        ...
        performed. The default mode is false.*/
    gscbool safe_mode () { return _op_check_parms; } 

    /*! Change the safe mode status. The default mode is false. */
    void safe_mode ( bool b ) { _op_check_parms=(char)b; }

    /*! Say if error messages should be printed or not (using gsout), default is false. */
   ...
   private : // IO ========================================================================================
    void _elemsave ( GsOutput& out, ElemType type, SeElement* first );
    void _elemload ( GsInput& inp, GsArray<SeElement*>& e, const GsArray<SeBase*>& s, GsManagerBase* man );
...
/*! This is the template version of the SeMeshBase class, that redefines
    the relevant methods of SeBase only for the purpose of including
    type casts to the user-defined classes. All methods are implemented
    inline just calling the corresponding method of the base class but 
    correctly applying type casts to convert default (base) types to
    the correct user types. */
template <class V, class E, class F> //, class VM, class EM, class FM>
class SeMesh : public SeMeshBase
 { public :
    typedef Se<V,E,F> S;
    typedef GsManager<V> VM;
    typedef GsManager<E> EM;
    typedef GsManager<F> FM;

    SeMesh () : SeMeshBase ( new VM, new EM, new FM ) {}
    SeMesh ( VM* vm, EM* em, FM* fm ) : SeMeshBase ( vm, em, fm ) {}
...
/*! \class SeMeshBase se_mesh.h
    ...
    to a mesh, the user must derive GsManagerBase, suply the required virtual
