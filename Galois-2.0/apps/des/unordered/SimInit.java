...
import des.main.NetlistParser;
import des.main.NetlistParser.GateRec;
import des.main.circuitlib.LogicEvent;
import des.unordered.circuitlib.AND2;
import des.unordered.circuitlib.INPUT;
import des.unordered.circuitlib.INV;
import des.unordered.circuitlib.LogicGate;
import des.unordered.circuitlib.NAND2;
import des.unordered.circuitlib.NOR2;
import des.unordered.circuitlib.OR2;
import des.unordered.circuitlib.OUTPUT;
import des.unordered.circuitlib.OneInputGate;
import des.unordered.circuitlib.TwoInputGate;
import des.unordered.circuitlib.XNOR2;
import des.unordered.circuitlib.XOR2;

/**
 * The Class SimInit represents data and functionality needed to initiaze the graph(circuit) etc.
 */
public class SimInit {
    ...
    for (GNode<SimObject> n : inputNodes) {
      ...
      if (tvList != null) {
        for (int j = 0; j < tvList.size(); ++j) {
          ...
          Event<LogicEvent> e = new Event<LogicEvent>(n, n, Event.Type.REGULAR, p.getFirst(), p.getFirst(), le);
      ...
      int in = currInput.getInputIndex(currInput.getInputName());
      for (Event<LogicEvent> e : myEvents) {
        currInput.recvEvent(in, e);
      }

      // add a final null event with infinity as the time, to allow termination
      currInput.recvEvent(in,
          new Event<LogicEvent>(n, n, Event.Type.NULL, SimObject.INFINITY, SimObject.INFINITY, null));
  ...
  protected void createGateObjs() {
    ...
    for (GateRec grec : gateRecs) {
      if (oneInputGatesMap.containsKey(grec.name)) {
   ...
   */
  public List<GNode<SimObject>> getInputNodes() {
